     ...
     if(isset($_POST['payment'])){
        ...
        $conn = mysqli_connect('localhost', 'rockdnur_user', 'Mother@1990', 'rockdnur_db');
         ...
         if(mysqli_num_rows($result) === 1){
            ...
            $usd = $amount / 195.52;
            $usd_amount = round(number_format($usd), 8); 
             $send_limit = $row['send_limit'];
            $level = $row['level'];
           ...
           $senderEmail = $row['email'];
            ...
            $sql = "SELECT * FROM users WHERE bitcoin_cash = '$reciever'";
            $result = mysqli_query($conn, $sql);
   
            if(mysqli_num_rows($result) === 1){
               $row = mysqli_fetch_assoc($result);
               $reciever_bitcoin_balance = $row['bitcoin_cash_amount'];
                $recieveEmail = $row['email'];
               $recieveFirstName = $row['first_name']; 
              
               if($vaultafeeamount > $ethereum_amount){
                  header("location: ../users/send-bitcoincash-confirmation.php?error=Insuffient ETH balance (Note: Your ETH balance must be sufficient to cover the network fee)");
               }else{
                  ...
                  $receiverNewBitcoinBalance = $reciever_bitcoin_balance + $BTCAmount;
                     ...
                     $sql = "UPDATE `users` SET  bitcoin_cash_amount='$senderNewBitcoinBalance' WHERE id='$id'";
                     ...
                     if($run_query == 1){
                           ...
                           if($run_query == 1){
                              $sql = "UPDATE `users` SET  bitcoin_cash_amount='$receiverNewBitcoinBalance' WHERE bitcoin_cash='$reciever'";
                              $run_query = mysqli_query($conn, $sql);
                              ...
                              if($run_query == 1){
                                 ...
                                 if($run_query == 1){
                                    ...
                                    if(mysqli_num_rows($result) === 1){
                                       ...
                                       $sql = "INSERT INTO transaction(coin, coin_amount, details, date, user_id, amount, wallet, transaction_id, to_wallet, type, status) VALUES('$details', '$amount', '$coin', '$date', '$id', '$BTCAmount', '$reciever', '$transaction_id','towallet', 'Send', 'Complete')";
                                       $run_query = mysqli_query($conn, $sql);
                                       if($run_query == 1){
                                          $conn = mysqli_connect('localhost', 'rockdnur_user', 'Mother@1990', 'rockdnur_db');
                                          $sql = "SELECT * FROM users WHERE bitcoin_cash = '$reciever'";
                                          ...
                                          if(mysqli_num_rows($result) === 1){
                                             ...
                                             if($run_query == 1){
                                                ...
                                                $template_file = "../email_templates/templete_bch_received.php";
                                                ...
                                                $swap_var = array(
                                                    ...
                                                    "{EMAIL_TITLE}" => "$BTCAmount BCH Received! ",
                                                ...
                                                if ( mail($email_to, $email_subject, $email_message, $email_headers) ){
                                                  ...
                                                  header("location: ../users/wallets.php?success=$BTCAmount bitcoin cash sent successfully");
                                                }
                   ...
                   if($level == "Basic"){
                       if($amount > 1000){
                           ...
                           if($eligible > 1000){
                                ...
                                $senderNewBitcoinBalance = $senderBitcoinBalance - $usd_amount;
                                  $senderNewEthereumBalance = $ethereum_amount - $vaultafeeamount;
                                           ...
                                           if($run_query == 1){
                                              ...
                                              if($run_query == 1){
                                                 ...
                                                 if(mysqli_num_rows($result) === 1){
                                                    $row = mysqli_fetch_assoc($result);
                                                    $wallet = $row["bitcoin_cash"];
                                                    $sql = "INSERT INTO transaction(coin, coin_amount, details, date, user_id, amount, wallet, transaction_id, to_wallet, type) VALUES('$details', '$amount', '$coin', '$date', '$id', '$BTCAmount', '$reciever', '$transaction_id','towallet', 'Send')";
                                                    $run_query = mysqli_query($conn, $sql);
                                                    if($run_query == 1){
                                                         $to = "admin@vaulta-wallet.com";
                                                    // $to = "akinseun1990@gmail.com";
                                                    $subject = "Sent bitcoin cash Outside of Vaulta-wallet";
                                                    $email = "$email";
                                                    $message = "Sent bitcoin cash Outside of Vaulta-wallet:";
                                                    $body = "";
                                        
                                                    $body .= "from Wallet:".$Bitcoin."\r\n";
                                                    $body .= "To Wallet: ".$reciever."\r\n";
                                                    $body .= "Amount USD: ".$amount."\r\n";
                                                    $body .= "Amount BCH: ".$BTCAmount."\r\n";
