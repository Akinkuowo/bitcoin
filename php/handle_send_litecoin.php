...
session_start();
   if(isset($_POST['payment'])){
       ...
       if($BTCAmount == ""){
            header("location: ../users/send-litecoin.php?error=Kindly input how much litecoin you want to send"); 
        }else{
            if($selectBTCWallet == ''){
                header("location: ../users/send-litecoin.php?error=Kindly select the wallet your are sending Ltc to");
            }else{
                ...
                if(mysqli_num_rows($result) === 1){
                    ...
                    $senderWalletAddress = $row['litecoin'];

                        
                    if($level == "Basic"){
                        if($amount < 10){
                            header("location: ../users/send-litecoin.php?error=The minimum amount you can send is $10");
                        }else{
                            if($selectBTCWallet == "vaulta-wallet"){
                                if($VaultaWalletReciever == ""){
                                    header("location: ../users/send-litecoin.php?error=Kindly enter reciever's wallet address");
                                }else{
                                     $conn = mysqli_connect('localhost', 'rockdnur_user', 'Mother@1990', 'rockdnur_db');
                                    $sql = "SELECT * FROM users WHERE litecoin = '$VaultaWalletReciever'";
                                    $result = mysqli_query($conn, $sql);
                                    ...
                                    if(mysqli_num_rows($result) === 1){
                                        ...
                                        $receiverBitcoinBalance = $row['litecoin_amount'];
                                       
                                            if( $senderWalletAddress == $VaultaWalletReciever){
                                                header("location: ../users/send-litecoin.php?error=You can't send Ltc to your Ltc wallet address");
                                            }else{
                                               if($BTCAmount > $senderBitcoinBalance){
                                                    header("location: ../users/send-litecoin.php?error=Insufficient litecoin Balance Kindly buy more litecoin to complete this transaction");
                                                }else{
                                                    $_SESSION['Amount'] = $amount;
                                                    $_SESSION['usd'] =  $BTCAmount;
                                                    $_SESSION['reciever'] =  $VaultaWalletReciever;
                                                    header("location: ../users/send-litecoin-confirmation.php");
                                                }
                                        ...
                                        header("location: ../users/send-litecoin.php?error=LTC Wallet Address can't be found");
                                    }
                                }
                                        ...
                                        // }else{
                                            ...
                                            if($BTCAmount > $senderBitcoinBalance){
                                                header("location: ../users/send-litecoin.php?error=Insufficient Litecoin Balance Kindly buy more litecoin to complete this transaction");
                                                    ...
                                                    // }else{
                                                    //     header("location: ../users/send-litecoin.php?error=Your daily limit as a bronze member is $10 to $15,000, You have sent LTC worth of $$send_limit, kindly upgrade your membership verification level to increase your transaction limit"); 
                                                    // }
